...
def _lmdb_tester(lmdb_path, is_sentinel2, size, test_attrs=None):
    key_re_checker = BEN_S2_RE if is_sentinel2 else BEN_S1_RE
    patch_class = BigEarthNet_S2_Patch if is_sentinel2 else BigEarthNet_S1_Patch
    ...
    with env.begin() as txn:
        ...
        for enc_patch_name, binary_patch_data in txn.cursor():
            ...
            patch_data = patch_class.loads(binary_patch_data)
            # just highlighting that the decoding has been succesfull
            assert isinstance(patch_data, patch_class)
